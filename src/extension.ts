...
export function activate(context: ExtensionContext) {
  let disposable = commands.registerCommand('extension.newBabelRepl', () => {
    repl = new Repl()
  })
...
class Repl {
  ...
  async init() {
    ...
    const { inputTextDocument, outputTextDocument } =
      await this.openTextDocuments(inputFilePath, outputFilePath)

    const { inputEditor, outputEditor } = await this.showTextDocuments(
      inputTextDocument,
      outputTextDocument
    )
  ...
  private async openTextDocuments(
    inputFilePath: string,
    outputFilePath: string
  ) {
    const inputUri = Uri.parse(path.normalize(`file://${inputFilePath}`))
    const outputUri = Uri.parse(path.normalize(`file://${outputFilePath}`))
  ...
  private showTextDocuments(
    inputTextDocument: TextDocument,
    outputTextDocument: TextDocument
  ) {
    const inputPromise = window.showTextDocument(
      inputTextDocument,
      ViewColumn.One
    )
    const outputPromise = window.showTextDocument(
      outputTextDocument,
      ViewColumn.Two,
      true
    )
